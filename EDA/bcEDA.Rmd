...
length(unique(bc_data2$sample_code_number))
```

We also don't really care about sample code number, as it is not a biological variable. Therefore, can remove. 
...
We will use only complete cases. Missing values can be imputed and we will see this later. For now, we will remove!

```{r}
bc_data4 <- bc_data3[complete.cases(bc_data3),]
```

**Save this data version for future use**. Saving data objects as RDS files reduce computation time making analysis efficient. 
...
Notice that we have more benign than malignant -- there is a class imbalance. We will deal with unbalanced datasets later
